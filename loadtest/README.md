...
### 4.0 复测快照（2026-09-14，层级 Agent 团队 + 质量门控全开后）

| 指标 | infra 基线 | chat 业务链路 |
| --- | --- | --- |
| 吞吐 | **458.39 req/s**（与首测一致） | 6 条流 / **0 失败**（15 VU 阶梯 3m50s，单流长致完成数少） |
| 延迟 p95 | **2.09ms**（首测 2.24ms） | 首 SSE 事件 p90 **43.8ms** / 整流 p90 **44.0s** |
| 总请求/错误 | 82,592 次 / **0%** | token 30,603 总计（≈5,100/条） |

> 复测要点：infra 数字与首测一致（框架无退化）；chat 整流从 13.8s→44.0s 是 Agent 链路从 3 次调用扩到 8 次（意图+领域路由+改写+生成+评审）的代价，非框架退化——瓶颈仍在 LLM 上游。落库统计（23 条完整链路）：均值 **15,640 tokens / 8 次 LLM 调用**，轻路径 524。

### 4.1 infra 基础设施基线（GET /health，无 LLM，空白对照组，2026-09-03 首测）
